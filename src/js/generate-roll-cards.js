...
const refs = {
  rollCardContainer: document.querySelector('.section-roll__container'),
  formRollSort: document.getElementById('sort-roll'),
  loadMoreBtn: document.querySelector('.load-more-btn-roll'),
};

export function addDomRollMarkup() {
  const rollMarkup = createRollMarkupArr(roll);
  refs.rollCardContainer.insertAdjacentHTML('afterbegin', rollMarkup);
}
...
export function selectRollHandle() {
  refs.loadMoreBtn.style.display = 'none';
  if (refs.formRollSort.value === 'name') {
    rollSortByName();
  } else if (refs.formRollSort.value === 'weight') {
    rollSortByWeight();
  } else if (refs.formRollSort.value === 'priceUp') {
    rollSortByPriceUp();
  } else if (refs.formRollSort.value === 'priceDown') {
    rollSortByPriceDown();
  }
}

function rollSortByName() {
  const rollByName = roll.sort((a, b) => a.name.localeCompare(b.name));
  const markup = createRollMarkupArr(rollByName);

  addDomRollMarkupSort(markup);
}

function rollSortByWeight() {
  const rollByWeight = roll.sort((a, b) => a.weight - b.weight);
  const markup = createRollMarkupArr(rollByWeight);

  addDomRollMarkupSort(markup);
}

function rollSortByPriceUp() {
  const rollByPriceUp = roll.sort((a, b) => a.price - b.price);
  const markup = createRollMarkupArr(rollByPriceUp);

  addDomRollMarkupSort(markup);
}

function rollSortByPriceDown() {
  const rollByPriceDown = roll.sort((a, b) => b.price - a.price);
  const markup = createRollMarkupArr(rollByPriceDown);

  addDomRollMarkupSort(markup);
}

function addDomRollMarkupSort(callback) {
  refs.rollCardContainer.innerHTML = '';
  refs.rollCardContainer.insertAdjacentHTML('afterbegin', callback);
}
